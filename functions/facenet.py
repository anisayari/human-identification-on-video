...
from functions.clustering import get_class_face

sys.path.insert(0, './functions')
sys.path.insert(0, './functions/yolov5')
...
def run(kmeans_model,
        ...
        dnn=False,
        color=[]# use OpenCV DNN for ONNX inference
        ):
    source = str(source)
    save_img = not nosave and not source.endswith('.txt')  # save inference images

    # Directories
    save_dir = './'  # increment run
    ...
    for path, img, im0s, vid_cap in tqdm(dataset):
        #if number_of_frame==10: FOR DEBUG PURPOSE
        #    break
        t1 = time_sync()
        if onnx:
